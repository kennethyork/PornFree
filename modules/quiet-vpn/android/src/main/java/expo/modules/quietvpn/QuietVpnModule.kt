...
class QuietVpnModule : Module() {
  ...
  override fun definition() = ModuleDefinition {
    ...
    AsyncFunction("startAsync") { options: StartOptions ->
      ...
      QuietVpnService.start(reactContext)
      status()
    }
    ...
    AsyncFunction("configureAsync") { options: StartOptions ->
      ...
      applyConfig(options)
      if (VpnRuntime.running) QuietVpnService.restart(reactContext)
      status()
    }
    ...
    AsyncFunction("setAlwaysOnVpnAsync") { options: EnableOptions ->
      ...
      // Lockdown is intentionally never enabled: Quiet only routes DNS, so blocking
  ...
  private fun adminComponent(context: Context) =
    ComponentName(context, QuietDeviceAdminReceiver::class.java)
}
