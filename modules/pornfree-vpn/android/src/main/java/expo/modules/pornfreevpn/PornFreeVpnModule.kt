...
class PornFreeVpnModule : Module() {
  ...
  override fun definition() = ModuleDefinition {
    ...
    AsyncFunction("startAsync") { options: StartOptions ->
      ...
      PornFreeVpnService.start(reactContext)
      status()
    }
    ...
    AsyncFunction("configureAsync") { options: StartOptions ->
      ...
      applyConfig(options)
      if (VpnRuntime.running) PornFreeVpnService.restart(reactContext)
      status()
    }
    ...
    AsyncFunction("stopAsync") { options: StopOptions ->
      ...
      PornFreeVpnService.stop(context)
    ...
    AsyncFunction("setAlwaysOnVpnAsync") { options: EnableOptions ->
      ...
      // Lockdown is intentionally never enabled: PornFree only routes DNS, so blocking
  ...
  private fun adminComponent(context: Context) =
    ComponentName(context, PornFreeDeviceAdminReceiver::class.java)
}
